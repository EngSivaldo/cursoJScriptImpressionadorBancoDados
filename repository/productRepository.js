...
class ProductRepository extends BaseRepository {
  ...
  async getAll() {
    try {
      const results = await super.getAll('product', ['id','name', 'size','price_in_size']); // Certifique-se de que o nome da tabela está correto
  ...
  async getById(id) {
    try {
      const result = await super.getById('product',  ['id','name', 'size','price_in_size'], id); // Certifique-se de que o nome da tabela está correto
